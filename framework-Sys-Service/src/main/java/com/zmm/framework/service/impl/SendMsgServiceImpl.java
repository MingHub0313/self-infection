...
public class SendMsgServiceImpl implements SendMsgService {
	...
	private void setParams(SendMsg sendMsg) {
		...
		switch (sendMsg.getBizType()) {
			// 用户注册验证码
			case "1":
				cacheKey2 = Constants.MsgChkType.REGISTER + sendMsg.getPhone();
				sendRandomCode(sendMsg, cacheKey2);
				break;
			// 登录确认验证码
			case "2":
				cacheKey1 = Constants.MsgChkType.LOGIN + DateUtil.getDate() + "_" + sendMsg.getPhone();
				cacheKey2 = Constants.MsgChkType.LOGIN + sendMsg.getPhone();
			...
			// 修改密码验证码
			case "3":
				cacheKey2 = Constants.MsgChkType.CHGPWD + sendMsg.getPhone();
				sendRandomCode(sendMsg, cacheKey2);
				break;
			// 身份验证验证码
			case "4":
				cacheKey2 = Constants.MsgChkType.VLDID + sendMsg.getPhone();
				sendRandomCode(sendMsg, cacheKey2);
				break;
			// 信息变更验证码
			case "5":
				cacheKey2 = Constants.MsgChkType.CHGINFO + sendMsg.getPhone();
				sendRandomCode(sendMsg, cacheKey2);
				break;
			// 活动确认验证码
			case "6":
				cacheKey2 = Constants.MsgChkType.AVTCMF + sendMsg.getPhone();
				sendRandomCode(sendMsg, cacheKey2);
				break;
			default:
				break;
